...
- **收藏**：遇到好用的提示词，点节点上「英文提示词（实际输出）」旁边的「收藏」按钮，命名后可选配一张出图截图，存进侧边栏「收藏」分页。
...
接上之后，节点内「英文提示词（实际输出）」上方会出现一条「上游输入」控制条：
...
**保存**：出了一张满意的图，点节点上「英文提示词（实际输出）」右侧的「收藏」按钮 → 填名字、可写备注、拖入或粘贴一张配图 → 保存。提示词正文只读展示，保存的就是节点当前实际输出。如果当前不在节点上，也可以从侧边栏「收藏」分页点「新建」，再选一个节点读取它的文本。
